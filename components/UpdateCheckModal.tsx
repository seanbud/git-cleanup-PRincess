    ...
    >
      <div className="flex flex-col items-center justify-center py-8 text-center">
        {state === "checking" && (
          <>
            <div
              className={`w-12 h-12 border-4 ${isPrincess ? "border-pink-200 border-t-pink-500" : "border-slate-700 border-t-blue-500"} rounded-full animate-spin mb-4`}
            />
            <p className={isPrincess ? "text-pink-900" : "text-slate-300"}>
        ...
        {state === "up-to-date" && (
          <>
            <div
              className={`w-12 h-12 flex items-center justify-center rounded-full ${isPrincess ? "bg-pink-100 text-pink-500" : "bg-green-500/20 text-green-400"} mb-4`}
            >
              <svg
                className="w-8 h-8"
                fill="none"
            ...
            <h3
              className={`text-lg font-bold mb-1 ${isPrincess ? "text-pink-900" : "text-white"}`}
            >
              You're all set!
            </h3>
            <p
              className={`text-sm ${isPrincess ? "text-pink-700" : "text-slate-400"}`}
            >
              You are already running the latest version of Git Cleanup
              PRincess.
            </p>
            <button
              onClick={onClose}
        ...
        {state === "found" && (
          <>
            <div
              className={`w-12 h-12 flex items-center justify-center rounded-full ${isPrincess ? "bg-pink-100 text-pink-500" : "bg-blue-500/20 text-blue-400"} mb-4 animate-bounce`}
            >
            ...
            <p
              className={`text-sm ${isPrincess ? "text-pink-700" : "text-slate-400"}`}
            >
              A new version is ready to sweep away those bugs.
            </p>
            <p
              className={`mt-2 text-xs opacity-60 ${isPrincess ? "text-pink-900" : "text-slate-300"}`}
            >
              Check the banner at the top of the app to start the download.
            </p>
            <button
              onClick={onClose}
        ...
        {state === "error" && (
          <>
            <div
              className={`w-12 h-12 flex items-center justify-center rounded-full bg-red-500/20 text-red-500 mb-4`}
            >
              <svg
                className="w-8 h-8"
                fill="none"
            ...
            <p className={isPrincess ? "text-pink-900" : "text-slate-300"}>
              Couldn't reach the kingdom's servers.
            </p>
            <button
              onClick={onClose}
