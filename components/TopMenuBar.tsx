...
const TopMenuBar: React.FC<TopMenuBarProps> = ({
        ...
        {items.map((item, i) =>
          item.separator ? (
            ...
            >
              ...
              {item.shortcut && (
                <span className="opacity-40 ml-4">{item.shortcut}</span>
              )}
            </div>
          ),
        )}
  ...
  return (
    <div
      className={`flex items-center w-full h-8 px-2 pr-[140px] ${bgClass} border-b border-black/5 select-none transition-colors duration-300 relative`}
    ...
    >
      ...
      >
        ...
        <div className="relative">
          <div
            className={topItemClass("Repository")}
            onClick={() =>
              setActiveMenu(activeMenu === "Repository" ? null : "Repository")
            }
          >
            Repository
          </div>
