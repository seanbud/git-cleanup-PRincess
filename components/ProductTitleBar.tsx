...
const ProductTitleBar: React.FC<ProductTitleBarProps> = ({
  mode,
  onToggleTheme,
}) => {
  ...
  return (
    <div
      className={`w-full h-12 border-b flex items-center px-6 z-30 transition-colors duration-500 ${bgClass}`}
    >
      <style>
        {`
          .font-chewy {
      ...
      >
        ...
        <span className="relative inline-flex items-baseline">
          ...
          <span>ince</span>
          <span
            className={`transition-all duration-300 overflow-hidden ${isPrincess ? "opacity-100 w-auto" : "opacity-0 w-0"}`}
          >
            ss
          </span>
        </span>
      ...
      <div className="ml-auto flex items-center">
        <button
          ...
          title={
            isPrincess ? "Switch to Prince Mode" : "Switch to Princess Mode"
          }
        >
