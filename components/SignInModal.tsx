...
const SignInModal: React.FC<SignInModalProps> = ({
  ...
  const startSignIn = async () => {
    try {
      ...
      const token = await GitHubAuthClient.pollForToken(
        data.device_code,
        data.interval,
      );
      if (token) {
      ...
      setError(
        "Failed to start authentication. Please check your internet connection.",
      );
      console.error(err);
    }
  };
    ...
    >
      <div className="text-center space-y-6">
        <div className="flex justify-center">
          <div
            className={`w-24 h-24 rounded-full flex items-center justify-center text-4xl shadow-inner ${isPrincess ? "bg-pink-100" : "bg-slate-800"}`}
          >
            👑
          </div>
        </div>
        ...
        <div>
          <h3
            className={`text-xl font-bold ${isPrincess ? "text-pink-600" : "text-blue-400"}`}
          >
            Ready to Clean Up?
          </h3>
          <p className="text-sm opacity-70 mt-2">
            Sign in with GitHub to access your repositories and start the
            cleanup process.
          </p>
        </div>
        ...
        {!authData ? (
          <button
            onClick={startSignIn}
            className={`w-full py-3 px-4 rounded-lg font-bold transition-all transform hover:scale-[1.02] active:scale-[0.98] ${
              isPrincess
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
            <div
              className={`p-4 rounded-xl border-2 border-dashed ${isPrincess ? "border-pink-200 bg-pink-50/50" : "border-slate-700 bg-slate-800/50"} relative group`}
            >
              <p className="text-xs uppercase font-bold opacity-50 mb-2">
