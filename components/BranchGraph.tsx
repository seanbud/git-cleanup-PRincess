...
const BranchGraph: React.FC<BranchGraphProps> = ({ mode, commits = [] }) => {
  ...
  if (commits.length === 0) {
    return (
      <div
        className={`p-4 border-b border-gray-100 flex items-center justify-center ${bgClass} overflow-hidden h-24 relative transition-colors duration-300`}
      >
        <div className="absolute top-1 left-2 text-[10px] uppercase font-bold text-gray-400">
          Network
        </div>
  ...
  return (
    <div
      className={`p-4 border-b border-gray-100 flex items-center ${bgClass} overflow-x-auto overflow-y-hidden h-24 relative transition-colors duration-300`}
    >
      <div className="absolute top-1 left-2 text-[10px] uppercase font-bold text-gray-400">
        Network
      </div>
