...
const NetworkGraph: React.FC<NetworkGraphProps> = ({
  ...
  const verticalLayout = useMemo(() => {
    if (!commits || commits.length === 0)
      return { nodes: [], paths: [], maxColumns: 1 };
    ...
    nodesList.forEach((node) => {
      node.parents.forEach((parentHash, i) => {
        ...
        const colorIdx =
          i === 0
            ? node.colorIndex
            : (parentNode?.colorIndex ??
              (node.colorIndex + 1) % BRANCH_COLORS.length);
  ...
  const filteredCommits = useMemo(() => {
    if (!currentBranch || commits.length === 0)
      return commits.slice(0, 15).reverse();
    const commitMap = new Map<string, CommitNode>();
    commits.forEach((c) => commitMap.set(c.hash, c));
    const headCommit =
      commits.find(
        (c) =>
          c.branch?.includes("HEAD -> " + currentBranch) ||
          c.branch?.includes(currentBranch),
      ) || commits[0];
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    const isAtEnd =
      target.scrollLeft + target.clientWidth >= target.scrollWidth - 20;
  ...
  if (commits.length === 0)
    return (
      <div
        className={`p-4 h-full flex items-center justify-center ${bgClass} text-xs text-gray-400`}
      >
        No commits found.
      </div>
    );
  ...
  if (!isExpanded) {
    ...
    const horizontalSvgWidth = Math.max(
      leftPadding + 200,
      (filteredCommits.length - 1) * nodeSpacing + leftPadding + 200,
    );
    const y = 35;
    return (
      <div className="relative h-24 flex-shrink-0">
        ...
        >
          <svg
            width={horizontalSvgWidth}
            height="70"
            className="pointer-events-none overflow-visible"
          >
            <SvgFilters />
            <path
              d={`M0,${y} L${horizontalSvgWidth},${y}`}
              stroke={lineColor}
              strokeWidth="5"
            ...
            {filteredCommits.length > 1 && (
              <path
                d={`M${leftPadding},${y} L${(filteredCommits.length - 1) * nodeSpacing + leftPadding},${y}`}
                stroke={lineColor}
                strokeWidth="6"
            ...
            {filteredCommits.map((commit, i) => {
              ...
              const isHead =
                commit.branch?.includes("HEAD ->") ||
                (i === filteredCommits.length - 1 && currentBranch);
              const allRefs =
                commit.branch?.split(",").map((r) => r.trim()) || [];
              const rawRef =
                allRefs.find(
                  (r) => r.startsWith("tag: ") || /v\d+\.\d+/.test(r),
                ) || "";
                  ...
                  {tagName && (
                    <g transform={`translate(${x}, ${y - 25})`}>
                      <rect
                        ...
                        rx={6}
                        fill={
                          isHead ? color1 : isPrincess ? "#fce7f3" : "#dbeafe"
                        }
                        filter="url(#lineShadow)"
                      />
                      <text
  ...
  return (
    <div
      className={`flex flex-col h-full border-t ${borderClass} ${bgClass} relative`}
    >
      <div
      ...
      <div className="flex-1 overflow-auto scrollbar-thin">
        ...
        >
          {/* Paths Layer */}
          <svg
            className="absolute top-0 left-0 pointer-events-none overflow-visible w-full h-full"
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      left: node.x - (node.isMerge ? MERGE_RADIUS : DOT_RADIUS),
                      top:
                        ROW_HEIGHT / 2 -
                        (node.isMerge ? MERGE_RADIUS : DOT_RADIUS),
                    }}
                  >
                    <div
                      className="rounded-full border-4 shadow-xl transition-transform group-hover:scale-110"
                      style={{
                        width: (node.isMerge ? MERGE_RADIUS : DOT_RADIUS) * 2,
                        height: (node.isMerge ? MERGE_RADIUS : DOT_RADIUS) * 2,
                  ...
                  {/* Commit Details */}
                  <div
                    className="flex text-[13px] items-center whitespace-nowrap overflow-hidden pr-20 pointer-events-none"
                    style={{ marginLeft: totalWidth - 40 }}
                  >
                    <span
                      className={`font-mono text-[11px] ${mutedTextClass} w-20 shrink-0 opacity-40`}
                    >
                    ...
                    {node.branch && (
                      <div className="flex shrink-0 gap-1 mr-4">
                        {node.branch.split(",").map((ref) => (
                          <span
                            key={ref}
                            className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tight shadow-md border-2 ${ref.includes("HEAD ->") ? (isPrincess ? "bg-pink-600 text-white border-pink-400" : "bg-blue-700 text-white border-blue-500") : isPrincess ? "bg-white text-pink-700 border-pink-100" : "bg-white text-blue-700 border-blue-100"}`}
                          >
                    ...
                    <span
                      className={`font-bold ${textColorClass} truncate mr-6 italic`}
                    >
                      {node.message}
                    </span>
                    <span
                      className={`text-[11px] font-black ${mutedTextClass} uppercase px-3 py-1 bg-white/60 rounded-full shadow-sm`}
                    >
                      {node.author}
                    </span>
                  </div>
