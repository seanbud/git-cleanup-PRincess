  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="" mode={mode}>
      <div className="flex flex-col items-center text-center py-2 -mt-4">
        {/* App Icon */}
        <div
          className={`w-28 h-28 flex items-center justify-center p-2 mb-4 drop-shadow-xl transform hover:scale-105 transition-transform duration-500`}
        >
          <img
            src="sprites/app-icon.png"
            alt="Git Cleanup PRincess"
        ...
        {/* App Name */}
        <h2
          className={`text-xl font-chewy tracking-wide mb-1 ${isPrincess ? "text-pink-700" : "text-blue-600"}`}
        >
        ...
        {/* Version */}
        <div
          className={`text-xs font-mono px-3 py-1 rounded-full mb-4 ${isPrincess ? "bg-pink-100 text-pink-600" : "bg-blue-100 text-blue-500"}`}
        >
          v{version}
        </div>
        ...
        {/* Description */}
        <p
          className={`text-sm mb-4 max-w-[280px] leading-relaxed ${isPrincess ? "text-pink-800/70" : "text-slate-500"}`}
        >
          A gamified Git cleanup tool fit for royalty. Stage, review, and clean
          your changes with style.
        </p>
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <button
            onClick={() => {
              // @ts-ignore
              window.electronAPI.openExternal(
                "https://github.com/seanbud/git-cleanup-PRincess/issues",
              );
            }}
            className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${isPrincess ? "border-pink-200 hover:bg-pink-50 text-pink-600" : "border-blue-200 hover:bg-blue-50 text-blue-500"}`}
          >
        ...
        {/* Update Check */}
        <div
          className={`w-full pt-4 border-t ${isPrincess ? "border-pink-100" : "border-slate-200"}`}
        >
          {updateStatus === "idle" && (
            <button
          ...
          {updateStatus === "checking" && (
            <div
              className={`text-xs flex items-center justify-center gap-2 ${isPrincess ? "text-pink-500" : "text-blue-500"}`}
            >
          ...
          {updateStatus === "available" && (
            <div
              className={`text-xs flex items-center justify-center gap-1 ${isPrincess ? "text-pink-600" : "text-blue-500"}`}
            >
