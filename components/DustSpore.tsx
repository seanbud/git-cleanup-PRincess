...
const DustSpore: React.FC<DustSporeProps> = React.memo(
  ({ fileName, linesAdded, linesRemoved, isScared, mode, delay }) => {
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        if (leftPupilRef.current) {
          leftPupilRef.current.setAttribute(
            "cx",
            (variant.leftEye.cx + x).toString(),
          );
          leftPupilRef.current.setAttribute(
            "cy",
            (variant.leftEye.cy + y).toString(),
          );
        }
        if (rightPupilRef.current) {
          rightPupilRef.current.setAttribute(
            "cx",
            (variant.rightEye.cx + x).toString(),
          );
          rightPupilRef.current.setAttribute(
            "cy",
            (variant.rightEye.cy + y).toString(),
          );
        }
      };
      ...
      `}</style>
        ...
        >
          ...
          `}
              ...
              >
                <span
                  className={`transition-all duration-300 ${isHovered ? "" : "truncate max-w-[80px]"}`}
                >
                  {shortName}
                </span>
              ...
              >
                ...
                >
                  {/* Variant Body Path */}
                  <path d={variant.path} fill={variant.color} />

                  {/* Eyes Container */}
                  <g
                    ref={eyesRef}
                    transform={isScared ? "scale(1.1) translate(-4, -2)" : ""}
                  >
                    {/* Left Eye */}
                    <circle
                  ...
                  <g opacity="0.6">
                    <ellipse
                      cx={variant.leftEye.cx - 10}
                      cy={variant.leftEye.cy + 12}
                      rx="4"
                      ry="2"
                      fill={blushColor}
                    />
                    <ellipse
                      cx={variant.rightEye.cx + 10}
                      cy={variant.rightEye.cy + 12}
                      rx="4"
                      ry="2"
                      fill={blushColor}
                    />
