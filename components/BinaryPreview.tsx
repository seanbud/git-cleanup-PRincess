...
const BinaryPreview: React.FC<BinaryPreviewProps> = ({
  filePath,
  fileStatus,
  mode,
}) => {
  ...
  const statusBadge = () => {
    ...
    if (fileStatus === FileStatus.MODIFIED)
      return (
        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-yellow-100 text-yellow-700 border border-yellow-200">
          Modified
        </span>
      );
    return null;
  ...
  return (
    <div className="flex items-center justify-center h-full p-8">
      <div
        className={`${cardBg} border ${cardBorder} rounded-2xl p-8 shadow-xl ${shadowColor} max-w-sm w-full text-center`}
      >
        {/* Large Icon */}
        ...
        {/* Decorative line */}
        <div
          className={`mt-6 h-px w-20 mx-auto ${isPrincess ? "bg-pink-200" : "bg-blue-200"}`}
        />
