...
const Character: React.FC<CharacterProps> = ({ mode, state, showBackdrop }) => {
  ...
  const scales: Record<string, number> = {
    ...
    "prince-idle.png": 1.1, // Another 5% bump (total 10% from base)
  ...
  const renderBackdrop = () => {
    ...
    const strokeColor = isPrincess
      ? "rgba(249, 168, 212, 0.05)"
      : "rgba(125, 211, 252, 0.05)";
    const wobblyPath =
      "M35,25 Q25,25 20,35 Q15,40 18,50 Q12,60 15,70 Q10,80 18,90 Q20,100 30,105 Q40,115 55,110 Q65,115 75,110 Q85,108 90,95 Q98,85 92,75 Q96,65 90,55 Q92,45 85,35 Q80,25 70,28 Q60,18 50,22 Q40,15 35,25 Z";
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center pointer-events-none overflow-visible">
      ...
      >
        ...
        <svg
          viewBox="0 0 100 120"
          className="absolute top-0 left-0 w-full h-full overflow-visible z-0 opacity-50"
        >
          {renderBackdrop()}
        </svg>
        ...
        <div
          key={`${mode}-${state}`}
          className="relative z-10 w-full h-full flex items-end justify-center animate-character-swap"
        >
          <img
            src={spritePath}
