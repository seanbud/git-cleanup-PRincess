...
const OptionsModal: React.FC<OptionsModalProps> = ({
  ...
  const labelClass =
    "block text-xs font-bold uppercase tracking-wider opacity-50 mb-1.5";
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Options" mode={mode}>
      <div className="flex min-h-[340px] -m-6">
        {/* Sidebar */}
        <div
          className={`w-44 shrink-0 border-r p-3 space-y-1 ${isPrincess ? "bg-slate-50/80 border-slate-200" : "bg-slate-900 border-slate-800"}`}
        >
          <div
            onClick={() => setActiveTab("accounts")}
            className={tabClass("accounts")}
          >
            <span>👤</span>
            <span>Accounts</span>
          </div>
        ...
        {/* Content Area */}
        <div
          className={`flex-1 flex flex-col min-w-0 ${isPrincess ? "bg-white" : "bg-slate-900"}`}
        >
          <div className="flex-1 p-6 overflow-y-auto">
            {activeTab === "accounts" && (
              <div className="space-y-5">
                <h3
                  className={`text-base font-bold ${isPrincess ? "text-slate-800" : "text-slate-100"}`}
                >
                  GitHub.com
                </h3>
                {user ? (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <div
                          className={`font-semibold text-sm ${isPrincess ? "text-slate-800" : "text-slate-100"}`}
                        >
                          {user.name || user.login}
                        </div>
                        <div
                          className={`text-xs ${isPrincess ? "text-slate-400" : "text-slate-500"}`}
                        >
                          @{user.login}
                        </div>
                      </div>
                  ...
                  <div
                    className={`text-center py-8 text-sm ${isPrincess ? "text-slate-400" : "text-slate-500"}`}
                  >
                    Not signed in.
                  </div>
                )}
            ...
            {activeTab === "integrations" && (
              <div className="space-y-5">
                <div>
                  ...
                  <div
                    className={`text-[10px] mb-2 ${isPrincess ? "text-pink-400" : "text-slate-500"}`}
                  >
                  ...
                  >
                    ...
                    {![
                      ...
                      "charm",
                      "notepad",
                    ].includes(localAppSettings.externalEditor) && (
                      <option value={localAppSettings.externalEditor}>
                        {localAppSettings.externalEditor} (Custom)
                      </option>
                    )}
                  </select>
                </div>
                <div>
                  ...
                  >
                    ...
                    {/* Allow keeping custom values if they don't match the list */}
                    {!["powershell", "cmd", "bash", "wsl"].includes(
                      localAppSettings.shell,
                    ) && (
                      <option value={localAppSettings.shell}>
                        {localAppSettings.shell} (Custom)
                      </option>
                    )}
                  </select>
                </div>
          ...
          {/* Footer */}
          <div
            className={`flex justify-end items-center space-x-3 px-6 py-3 border-t ${
              isPrincess
          ...
          >
            {showSaved && (
              <span
                className={`text-xs font-bold animate-pulse ${isPrincess ? "text-pink-500" : "text-blue-400"}`}
              >
                Saved ✓
              </span>
            )}
            ...
            <button
              onClick={async () => {
                ...
                if (localGitConfig.name !== gitConfig.name) {
                  // @ts-ignore
                  await window.electronAPI.gitCmd([
                    "config",
                    "user.name",
                    localGitConfig.name,
                  ]);
              ...
              className={`px-6 py-2 rounded-lg text-sm font-bold text-white shadow-lg transition-transform active:scale-[0.98] ${
                isPrincess
