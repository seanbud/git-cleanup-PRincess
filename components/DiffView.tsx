...
function isBinaryFile(filePath: string, diffContent?: string): boolean {
  ...
  if (
    ...
    (diffContent.includes("Binary files") ||
      diffContent.includes("GIT binary patch"))
  )
    return true;
  return false;
}
...
const DiffLineItem = React.memo<DiffLineItemProps>(
  ({ line, isPrincess, filePath, mode }) => {
    ...
    return (
      <div
        className={`${lineBgClass} flex hover:brightness-[0.98] transition-all group border-l-2 ${line.type === "added" ? "border-emerald-400" : line.type === "removed" ? "border-rose-400" : "border-transparent"}`}
      >
        {/* Line Numbers */}
        <div className="flex shrink-0 select-none bg-gray-50/80 border-r border-gray-200/50 font-mono text-[10px]">
          <div className="w-10 text-right pr-2 py-[2px] text-gray-400 opacity-70 group-hover:opacity-100">
            {oldNum}
          </div>
          <div className="w-10 text-right pr-2 py-[2px] text-gray-400 opacity-70 group-hover:opacity-100 border-l border-gray-200/30">
            {newNum}
          </div>
        </div>
        ...
        {/* Code Content */}
        <div
          className={`px-4 py-[2px] whitespace-pre-wrap break-all ${textClass} font-mono leading-relaxed w-full min-w-0 flex items-start`}
        >
          <span
            className={`w-4 shrink-0 select-none font-bold ${indicatorColor}`}
          >
            {line.text.charAt(0) === "+"
              ? "+"
              : line.text.charAt(0) === "-"
...
const DiffView: React.FC<DiffViewProps> = ({ file, mode }) => {
  ...
  const [expandedAutoFolds, setExpandedAutoFolds] = useState<Set<string>>(
    new Set(),
  );
      ...
      for (let i = 0; i < lineBuffer.length; i++) {
        ...
        if (line.type === "context") {
          ...
          if (unchangedCount > FOLD_THRESHOLD) {
            result.push({
              type: "lines",
              lines: subBuffer.slice(0, CONTEXT_LINES),
            ...
            result.push({
              type: "lines",
              lines: subBuffer.slice(-CONTEXT_LINES),
              chunkId: currentChunkId,
            });
  ...
  if (!file) {
    return (
      <div
        className={`flex flex-col items-center justify-center h-full text-gray-400 ${bgClass} select-none`}
      >
        <div
          className={`w-20 h-20 mb-4 rounded-full ${headerBgClass} flex items-center justify-center shadow-inner`}
        >
          <svg
            width="32"
            height="32"
        ...
        <p className="font-chewy text-lg tracking-wide opacity-60 text-center px-10">
          Select a file to witness the magical transformations
        </p>
      </div>
    );
  }
  ...
  if (isImageFile(file.path)) {
    return (
      <ImagePreview filePath={file.path} fileStatus={file.status} mode={mode} />
    );
  }
  ...
  return (
    <div
      className={`flex flex-col h-full ${bgClass} font-mono text-xs transition-colors duration-300 relative overflow-hidden`}
    >
      {/* Top Header */}
      <div
        className={`px-4 py-3 border-b border-black/10 ${headerBgClass} flex items-center justify-between sticky top-0 z-20 backdrop-blur-md bg-opacity-90 shadow-md`}
      >
        <div className="flex items-center space-x-3 overflow-hidden">
          <div
            className={`p-1.5 rounded-lg ${isPrincess ? "bg-pink-200 text-pink-700" : "bg-blue-200 text-blue-700"}`}
          >
            <svg
              width="16"
              height="16"
              ...
              case "chunk-header": {
                ...
                return (
                  ...
                  >
                    ...
                    {isCollapsed && (
                      <span className="ml-auto bg-white/40 px-2 py-0.5 rounded text-[8px] uppercase tracking-widest text-gray-500">
                        Collapsed
                      </span>
                    )}
                  </button>
