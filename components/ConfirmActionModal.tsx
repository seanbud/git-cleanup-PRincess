...
const ConfirmActionModal: React.FC<ConfirmActionModalProps> = ({
  ...
  const getGitCommands = () => {
    if (isRemove) {
      ...
      return filePaths
        .map((p) => [`git reset HEAD -- "${p}"`, `git checkout -- "${p}"`])
        .flat();
    }
  };
  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* Description */}
        <p
          className={`text-sm leading-relaxed ${isPrincess ? "text-slate-600" : "text-slate-300"}`}
        >
          {description}
        </p>
          ...
          {showDetails && (
            ...
            >
              <div className="p-3 overflow-x-auto max-h-40 overflow-y-auto">
                <pre
                  className={`text-[11px] leading-relaxed font-mono whitespace-pre ${
                    isPrincess ? "text-slate-600" : "text-slate-300"
                  }`}
                >
                  {getGitCommands().join("\n")}
                </pre>
