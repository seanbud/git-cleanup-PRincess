...
const ImagePreview: React.FC<ImagePreviewProps> = ({
  ...
  const [viewMode, setViewMode] = useState<"side-by-side" | "swipe">(
    "side-by-side",
  );
    ...
    const loadImages = async () => {
      ...
      try {
        ...
        if (fileStatus !== FileStatus.ADDED) {
          // @ts-ignore
          const headResult =
            await window.electronAPI.getGitFileBase64(filePath);
    ...
    return (
      <div className="flex items-center justify-center h-full text-gray-400">
        <div className="flex flex-col items-center gap-3">
          <div
            className={`w-8 h-8 border-2 border-${accentColor}-300 border-t-transparent rounded-full animate-spin`}
          />
  ...
  if (isNewFile || isDeleted) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 gap-4">
        <span
          className={`px-3 py-1 rounded-full text-xs font-bold bg-${badgeColor}-100 text-${badgeColor}-700 border border-${badgeColor}-200`}
        >
          {badge}
        </span>
        {image && (
          <>
            <div
              className={`relative rounded-xl border-2 border-dashed ${isPrincess ? "border-pink-200" : "border-blue-200"} p-2 bg-white shadow-lg max-w-[80%] max-h-[60vh]`}
            >
              <img
                src={image.dataUri}
                alt={filePath}
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div
        className={`flex items-center justify-center gap-2 p-2 border-b ${isPrincess ? "border-pink-100 bg-pink-50/50" : "border-blue-100 bg-blue-50/50"}`}
      >
        <button
      ...
      {viewMode === "side-by-side" ? (
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-4 border-r border-gray-200 bg-red-50/20">
            <span className="text-xs font-bold text-red-400 mb-3 uppercase tracking-wider">
              Before (HEAD)
            </span>
            {oldImage && (
              <>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-4 bg-green-50/20">
            <span className="text-xs font-bold text-green-500 mb-3 uppercase tracking-wider">
              After (Working Tree)
            </span>
            {newImage && (
              <>
                <div className="rounded-lg border border-green-200/50 p-1 bg-white shadow-sm max-w-full max-h-[50vh] overflow-hidden">
                  <img
                    src={newImage.dataUri}
                    alt="New version"
                    className="max-w-full max-h-[45vh] object-contain"
                  />
                </div>
                <span className="text-[10px] text-gray-400 mt-2">
                  {formatBytes(newImage.size)}
                </span>
              </>
            )}
          </div>
        ...
        <div className="flex-1 relative overflow-hidden flex items-center justify-center p-4">
          <div className="relative max-w-[80%] max-h-[60vh]">
            ...
            >
              <div
                className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-6 h-6 rounded-full bg-white border-2 border-${accentColor}-400 shadow-md flex items-center justify-center`}
              >
                <svg
                  width="10"
                  height="10"
      ...
      {oldImage && newImage && (
        <div
          className={`flex justify-center gap-6 py-2 border-t text-[10px] ${isPrincess ? "border-pink-100 bg-pink-50/30" : "border-blue-100 bg-blue-50/30"}`}
        >
          <span className="text-gray-500">
            Size change: {formatBytes(oldImage.size)} →{" "}
            {formatBytes(newImage.size)}{" "}
            <span
              className={
                newImage.size > oldImage.size
                  ? "text-red-500"
                  : "text-green-500"
              }
            >
              ({newImage.size > oldImage.size ? "+" : ""}
              {formatBytes(Math.abs(newImage.size - oldImage.size))})
            </span>
          </span>
        </div>
      )}
