        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 text-xs font-medium cursor-pointer flex items-center justify-between border-none transition-colors ${isSelected ? 'bg-gray-100 text-gray-900' : 'hover:bg-gray-50 text-gray-600'}`}
              onClick={(e) => {
                e.stopPropagation();
                onSelect(item);
                setActiveDropdown(null);
      ...
      >
        ...
        {activeDropdown === 'REPO' && renderDropdownList(
          ...
          <>
            ...
            <button
              type="button"
              className="w-full text-left px-4 py-2.5 text-xs font-medium cursor-pointer hover:bg-gray-50 text-blue-600 flex items-center gap-2 bg-transparent border-none"
              onClick={(e) => {
                e.stopPropagation();
                onOpenRepo?.();
                setActiveDropdown(null);
      ...
      <div className="flex-1 flex items-center min-w-0 z-40">
        ...
        >
          ...
          <button
            ...
            className={`text-lg md:text-xl font-bold cursor-pointer hover:underline decoration-2 decoration-dotted underline-offset-4 flex items-center ${branchText} bg-transparent border-none p-0`}
          >
