        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full px-4 py-2.5 text-xs font-medium cursor-pointer flex items-center justify-between border-none bg-transparent outline-none focus:bg-gray-50 ${isSelected ? 'bg-gray-100 text-gray-900' : 'hover:bg-gray-50 text-gray-600'}`}
              onClick={(e) => {
                e.stopPropagation();
                onSelect(item);
                setActiveDropdown(null);
      ...
      >
        ...
        {activeDropdown === 'REPO' && renderDropdownList(
          ...
          <>
            ...
            <button
              type="button"
              className="w-full px-4 py-2.5 text-xs font-medium cursor-pointer hover:bg-gray-50 text-blue-600 flex items-center gap-2 border-none bg-transparent outline-none focus:bg-gray-50"
              onClick={(e) => {
                e.stopPropagation();
                onOpenRepo?.();
                setActiveDropdown(null);
      ...
      <div className="flex-1 flex items-center min-w-0 z-40">
        ...
        >
          ...
          <button
            ...
            className={`bg-transparent border-none p-0 text-lg md:text-xl font-bold cursor-pointer hover:underline decoration-2 decoration-dotted underline-offset-4 flex items-center outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-4 rounded ${branchText}`}
          >
