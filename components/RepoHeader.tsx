...
const RepoHeader: React.FC<RepoHeaderProps> = ({
  ...
  const renderDropdownList = (items: string[], onSelect: (item: string) => void, selectedItem: string, extraItems?: React.ReactNode) => (
    <div className="absolute top-full mt-2 left-0 w-[280px] bg-white rounded-lg shadow-xl border border-gray-200 ring-1 ring-black/5 z-50 overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-1 duration-100" role="listbox">
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full px-4 py-2.5 text-xs font-medium cursor-pointer flex items-center justify-between border-none bg-transparent ${isSelected ? 'bg-gray-100 text-gray-900' : 'hover:bg-gray-50 text-gray-600'}`}
              onClick={(e) => {
                e.stopPropagation();
                onSelect(item);
                setActiveDropdown(null);
      ...
      >
        ...
        {activeDropdown === 'REPO' && renderDropdownList(
          ...
          <>
            ...
            <button
              type="button"
              className="w-full px-4 py-2.5 text-xs font-medium cursor-pointer hover:bg-gray-50 text-blue-600 flex items-center gap-2 border-none bg-transparent"
              onClick={(e) => {
                e.stopPropagation();
                onOpenRepo?.();
                setActiveDropdown(null);
