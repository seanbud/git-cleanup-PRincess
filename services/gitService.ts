...
export class GitService {
    ...
    static async getStatusFiles(comparisonBranch?: string): Promise<GitFile[]> {
        ...
        // Optimization: Parallelize all Git data fetching calls
    ...
    static async removeFiles(filePaths: string[]): Promise<boolean> {
        ...
        // 1. Remove from git index first (bulk operation)
        ...
        const trashPromises = filePaths.map(path => (window.electronAPI as any).trashFile(path));
