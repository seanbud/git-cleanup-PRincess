...
export class GitService {
    ...
    static async getBestComparisonBranch(preFetchedUpstream?: string, preFetchedBranches?: string[]): Promise<string> {
        // 1. Try upstream
        const upstream = preFetchedUpstream !== undefined ? preFetchedUpstream : await this.getUpstreamBranch();
        if (upstream) return upstream;

        // 2. Try common branches
        const common = ['develop', 'development', 'main', 'master'];
        const branches = preFetchedBranches !== undefined ? preFetchedBranches : await this.getBranches();
    ...
    static async getStatusFiles(comparisonBranch?: string, preFetchedCurrentBranch?: string): Promise<GitFile[]> {
        ...
        // Parallelize independent Git operations to reduce overhead
        const [statusRes, currentBranch, localStats, stagedStats] = await Promise.all([
            git('status', '--porcelain'),
            preFetchedCurrentBranch !== undefined ? Promise.resolve(preFetchedCurrentBranch) : this.getCurrentBranch(),
        ...
        // Parallelize branch-specific diffs if needed
        const [diffRes, branchStats] = isDifferentBranch
            ? await Promise.all([
        ...
        if (diffRes && diffRes.success && diffRes.stdout.trim()) {
        ...
        if (branchStats && branchStats.success) addStats(branchStats.stdout);
    ...
    static async getGitConfig(): Promise<GitConfig> {
        // Parallelize IPC calls for git config lookups
