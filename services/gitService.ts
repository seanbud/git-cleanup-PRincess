...
export class GitService {
    ...
    static async getBestComparisonBranch(branches?: string[], currentBranch?: string): Promise<string> {
        // 1. Try upstream
        ...
        const availableBranches = branches || await this.getBranches();
        const activeBranch = currentBranch || await this.getCurrentBranch();

        for (const target of common) {
            if (availableBranches.includes(target)) {
                // Verify it's not the same branch
                if (activeBranch !== target) return target;
            }
        }
        ...
        if (remoteRes.success) {
            ...
            for (const target of common) {
                ...
                if (remoteBranches.some((rb: string) => rb.includes(rTarget))) {
                    if (activeBranch !== rTarget) return rTarget;
                }
            }
        }
    ...
    static async getStatusFiles(comparisonBranch?: string): Promise<GitFile[]> {
        ...
        // Optimization: Parallelize stat fetching to reduce multiple sequential process spawns
        ...
        if (comparisonBranch && comparisonBranch !== currentBranch) {
            statPromises.push(git('diff', '--numstat', '--text', `${comparisonBranch}...HEAD`));
        }
    ...
    static async getGitConfig(): Promise<GitConfig> {
        // Optimization: Parallelize IPC calls to reduce latency
    ...
    static async discardChanges(filePaths: string[], comparisonBranch?: string): Promise<boolean> {
        ...
        // If restoring FROM a base branch (making them match the base)
        if (comparisonBranch && comparisonBranch !== currentBranch) {
