...
export class GitService {
    ...
    static async getStatusFiles(comparisonBranch?: string): Promise<GitFile[]> {
        ...
        // Optimization: Parallelize initial Git commands to reduce command waterfall.
        // We fetch status, current branch, and local/staged stats concurrently.
        ...
        // 2. Get committed differences and stats if comparing to another branch
        let diffRes = null;
        let branchStats = null;

        if (comparisonBranch && comparisonBranch !== currentBranch) {
            // Optimization: Parallelize comparison-based diff and numstat calls.
        ...
        // Apply uncommitted stats
        if (localStats.success) addStats(localStats.stdout);
        if (stagedStats.success) addStats(stagedStats.stdout);

        // Apply comparison branch stats
        if (branchStats && branchStats.success) {
            addStats(branchStats.stdout);
        }
    ...
    static async getGitConfig(): Promise<GitConfig> {
        // Optimization: Parallelize IPC calls to reduce latency from 3 serial round-trips to 1 parallel batch.
