...
export class GitService {
  ...
  static async getBestComparisonBranch(): Promise<string> {
    ...
    if (remoteRes.success) {
      const remoteBranches = remoteRes.stdout
        .split("\n")
        .map((b: string) => b.trim());
      for (const target of common) {
  ...
  static async getGitConfig(): Promise<GitConfig> {
    ...
    // @ts-ignore
    const defaultBranch =
      (await window.electronAPI.gitConfigGet("init.defaultBranch")) || "main";
