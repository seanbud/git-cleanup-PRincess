...
export class GitService {
    ...
    static async getBestComparisonBranch(preFetched?: { branches?: string[], current?: string, upstream?: string }): Promise<string> {
        // 1. Try upstream
    ...
    static async getStatusFiles(comparisonBranch?: string, currentBranch?: string): Promise<GitFile[]> {
        ...
        // BOLT OPTIMIZATION: Parallelize independent Git commands to minimize UI refresh latency.
        // Instead of waiting for status -> diff -> numstat sequentially (~150ms+), we run them in parallel (~50ms total).
        const [statusRes, comparisonDiffRes, localStats, stagedStats, branchStats] = await Promise.all([
            git('status', '--porcelain'),
            (comparisonBranch && comparisonBranch !== curBranch) ? git('diff', '--name-status', `${comparisonBranch}...HEAD`) : Promise.resolve({ success: false, stdout: '' }),
            git('diff', '--numstat', '--text'),
            git('diff', '--numstat', '--text', '--cached'),
            (comparisonBranch && comparisonBranch !== curBranch) ? git('diff', '--numstat', '--text', `${comparisonBranch}...HEAD`) : Promise.resolve({ success: false, stdout: '' })
        ]);

        // 1. Process Status Results
        ...
        // 2. Process Comparison Differences
        if (comparisonDiffRes.success && comparisonDiffRes.stdout.trim()) {
            const lines = comparisonDiffRes.stdout.split('\n').filter(Boolean);
            lines.forEach((line: string) => {
    ...
    /**
     * BOLT OPTIMIZATION: Bulk restore files to minimize process spawning.
     */
    ...
    /**
     * BOLT OPTIMIZATION: Bulk remove files. Batches git command and parallelizes Electron IPC.
     */
    static async removeFiles(filePaths: string[]): Promise<boolean> {
        ...
        const results = await Promise.all(filePaths.map(async (filePath) => {
            // @ts-ignore
            return await window.electronAPI.trashFile(filePath);
        }));
