...
export class GitService {
    ...
    /**
     * Optimization: Accept optional parameters to avoid redundant Git CLI calls.
     */
    static async getBestComparisonBranch(currentBranchName?: string, branchList?: string[]): Promise<string> {
        ...
        // 4. Fallback to self
        return currentBranchName || await this.getCurrentBranch();
    }
    ...
    /**
     * Optimization: Use Promise.all to parallelize multiple independent Git commands.
     * This significantly reduces the sequential latency (waterfall) during UI refreshes.
     */
    static async getStatusFiles(comparisonBranch?: string, currentBranchName?: string): Promise<GitFile[]> {
        ...
        const [statusRes, diffRes, localStats, stagedStats, branchStats] = await Promise.all([
    ...
    /**
     * Optimization: Use Promise.all to parallelize multiple independent IPC calls.
     * This reduces total latency during startup and settings refreshes.
     */
    ...
    /**
     * Optimization: Perform bulk Git restores to avoid O(N) sequential loop overhead.
     */
    static async restoreFiles(filePaths: string[], comparisonBranch?: string): Promise<boolean> {
        ...
        // 1. Bulk unstage everything in the list
        await git('reset', 'HEAD', '--', ...filePaths);

        // 2. Bulk restore working tree for tracked files (Modified/Deleted)
    ...
    /**
     * Optimization: Perform bulk Git removals and parallelized Electron trash calls
     * to avoid O(N) sequential loop overhead in the renderer.
     */
    static async removeFiles(filePaths: string[]): Promise<boolean> {
        if (filePaths.length === 0) return true;

        // 1. Bulk remove from git index
        await git('rm', '--cached', '-f', '--ignore-unmatch', ...filePaths);

        // 2. Parallelized move to trash
