...
export class GitService {
    ...
    static async getBestComparisonBranch(upstreamArg?: string, branchesArg?: string[], currentBranchArg?: string): Promise<string> {
        // 1. Try upstream
        const upstream = upstreamArg !== undefined ? upstreamArg : await this.getUpstreamBranch();
        if (upstream) return upstream;

        // 2. Try common branches
        const common = ['develop', 'development', 'main', 'master'];
        const branches = branchesArg !== undefined ? branchesArg : await this.getBranches();

        for (const target of common) {
            if (branches.includes(target)) {
                // Verify it's not the same branch
                const current = currentBranchArg !== undefined ? currentBranchArg : await this.getCurrentBranch();
        ...
        // 4. Fallback to self
        return currentBranchArg !== undefined ? currentBranchArg : await this.getCurrentBranch();
    }
    ...
    static async getStatusFiles(comparisonBranch?: string, currentBranchArg?: string): Promise<GitFile[]> {
        const currentBranch = currentBranchArg !== undefined ? currentBranchArg : await this.getCurrentBranch();
    ...
    static async getDiff(filePath: string, comparisonBranch?: string, currentBranchArg?: string): Promise<string> {
        const currentBranch = currentBranchArg !== undefined ? currentBranchArg : await this.getCurrentBranch();
    ...
    static async restoreFiles(filePaths: string[], comparisonBranch?: string, currentBranchArg?: string): Promise<boolean> {
