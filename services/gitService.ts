...
export class GitService {
    ...
    static async getBestComparisonBranch(preFetched?: { currentBranch?: string; branches?: string[] }): Promise<string> {
        // 1. Try upstream
        ...
        const current = preFetched?.currentBranch || await this.getCurrentBranch();
    ...
    static async getStatusFiles(comparisonBranch?: string, preFetchedBranch?: string): Promise<GitFile[]> {
        const files: GitFile[] = [];
        const seenPaths = new Set<string>();
        const currentBranch = preFetchedBranch || await this.getCurrentBranch();
        const hasComparison = comparisonBranch && comparisonBranch !== currentBranch;

        // Optimization: Parallelize all Git commands to reduce latency
