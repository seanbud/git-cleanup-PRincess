...
export class GitService {
    ...
    static async getBestComparisonBranch(preFetched?: { currentBranch: string; upstream: string; branches: string[] }): Promise<string> {
        // 1. Try upstream
    ...
    static async getGitConfig(): Promise<GitConfig> {
        // Parallelize fetching multiple config values
    ...
    static async removeFiles(filePaths: string[]): Promise<boolean> {
        ...
        return rmRes.success && results.every(res => res.success);
    }
