...
export class GitService {
    ...
    static async getStatusFiles(comparisonBranch?: string): Promise<GitFile[]> {
        // Parallelize initial git calls to reduce command waterfall
        const [currentBranch, statusRes, localStats, stagedStats] = await Promise.all([
        ...
        const isDifferentBranch = comparisonBranch && comparisonBranch !== currentBranch;

        // 2. Concurrently fetch branch differences if comparing to another branch
        let diffRes = { success: false, stdout: '' };
        let branchStats = { success: false, stdout: '' };

        if (isDifferentBranch) {
            [diffRes, branchStats] = await Promise.all([
        ...
        if (isDifferentBranch && diffRes.success && diffRes.stdout.trim()) {
        ...
        if (isDifferentBranch && branchStats.success) addStats(branchStats.stdout);
    ...
    static async getGitConfig(): Promise<GitConfig> {
        // Parallelize multiple independent IPC calls to reduce total latency
    ...
    /**
     * Bulk restore files to their state in the comparison branch or HEAD.
     * Optimized to use single Git commands for multiple paths.
     */
    ...
    /**
     * Bulk remove files from the Git index and move them to the trash.
     * Optimized to use a single Git command and parallelize disk operations.
     */
    static async removeFiles(filePaths: string[]): Promise<boolean> {
        if (filePaths.length === 0) return true;

        // 1. Bulk remove from git index (keep on disk initially)
