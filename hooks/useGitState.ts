...
export function useGitState(): UseGitStateReturn {
  ...
  const handleAction = useCallback(
    async (
      ...
      setCharacterState(
        actionType === "RESTORE"
          ? CharacterState.ACTION_GOOD
          : CharacterState.ACTION_BAD,
      );

      const selectedFiles = gitState.files.filter((f) =>
        gitState.selectedFileIds.has(f.id),
      );
