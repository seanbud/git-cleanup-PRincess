...
## 2026-02-24 - Path Traversal and Shell Injection via IPC
**Vulnerability:** IPC handlers allowed arbitrary file paths and shell commands to be passed from the renderer to the main process, risking path traversal and secondary command injection.
**Learning:** Even when using `execFile`, if the binary being executed (like a shell) can itself execute further commands, an allowlist of permitted binaries/shells is necessary. Similarly, file system operations must be restricted to the active workspace to prevent unauthorized access.
**Prevention:** Implement an allowlist for shells (`isValidShell`) and a path traversal check (`isSafePath`) in the main process. Validate all configuration strings (e.g., length and character limits) before persisting or using them.
