...
import fs from 'fs';
import { isSafePath, isValidShell, isValidEditor } from '../utils/security';
...
function saveSettings(settings: any) {
    // Security: Validate shell and editor before saving
    if (settings.shell && !isValidShell(settings.shell)) {
        delete settings.shell;
    }
    if (settings.externalEditor && !isValidEditor(settings.externalEditor)) {
...
app.whenReady().then(() => {
    ...
    ipcMain.handle('shell:open-editor', async (_, filePath: string) => {
        ...
        // Security: Validate editor binary
    ...
    ipcMain.handle('shell:trash-item', async (_, filePath: string) => {
        ...
        if (!isSafePath(currentCwd, fullPath)) {
